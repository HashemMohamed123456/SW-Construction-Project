  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const TextCustom(text: "Edit Task",fontWeight: FontWeight.bold,fontSize: 20,color: Colors.white,),
        centerTitle: true,
              ...
              children: [
                  ...
                  child: ElevatedButton(
                      ...
                      onPressed: (){
                        if(TodoCubit.get(context).formKey.currentState!.validate()){
                          TodoCubit.get(context).editTask().then((value){
                            Navigator.pop(context);
                          });
                        }
                      },
                      child:const TextCustom(text: "Edit Task",fontWeight: FontWeight.bold,fontSize: 20,color: Colors.white) ),
                ),
                ...
                Expanded(
                  child: ElevatedButton(
                    ...
                    onPressed: (){
                    if(TodoCubit.get(context).formKey.currentState!.validate()){
                      TodoCubit.get(context).deleteTask().then((value){
                        Navigator.pop(context);
                      });
                    }
                    },
                    child:const TextCustom(text: "Delete Task",fontWeight: FontWeight.bold,fontSize: 20,color: Colors.white) ),
                ),
              ],
            ),
          )
